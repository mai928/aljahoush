                    ...
                    sliderData.map((slide, index) => (
                        <SwiperSlide key={index} className="swiper-slide relative">
                            <div className='relative'>
                                <img className='w-full h-[70vb] lg:h-[130vb]' loading='eager' alt='img' src={slide.photo} />
                                <div className='absolute top-0 bottom-0 right-0 left-0 bg-black bg-opacity-40'></div>

                            </div>
                            <div key={index} className='absolute top-7 lg:top-44 start-10 lg:start-64  lg:w-[55%]'>
                                <h2 className='font-IntoLightTw0 text-lg lg:text-8xl text-white'>{t(slide.title)}</h2>
                                ...
                                <div className=' lg:mt-16 mb-10 text-white  font-Outfit text-[12px] lg:text-[17px]  font-semibold lg:font-bold w-[80%]' dangerouslySetInnerHTML={{__html:DOMPurify.sanitize(t(slide?.details))}} />
                                <Link to={'/'} className='bg-primary_color font-Outfit text-base lg:text-xl  text-color_heading  lg:font-[600] py-2 lg:py-5 px-4 lg:px-10 rounded-md hover:bg-secondary_color'>{t("Discover More")}</Link>
                            </div>
                        </SwiperSlide>
                    ))
                }
